...
class BinaryCrossEntropy(nn.Module):
    ...
    def forward(self, y_pred, y_true, weight=None):
        """
        Args:
            y_pred: Prediction probabilities of shape (n, 2)
            y_true: True binary labels of shape (n, 2)
            weight: sample weights to adjust the sample loss values
        Returns:
            A loss scalar.
        """
        # bce_loss_fn = nn.BCELoss(reduction=self.reduce)  # Create an instance of BCELoss
        bce_loss_fn = nn.BCEWithLogitsLoss()
        bce_loss = bce_loss_fn(y_pred, y_true)

        # Compute BCE loss for each class separately
        # bce_loss_class_0 = bce_loss_fn(y_pred[:, 0], y_true[:, 0])
        # bce_loss_class_1 = bce_loss_fn(y_pred[:, 1], y_true[:, 1])

        # Average the losses
        # bce_loss = (bce_loss_class_0 + bce_loss_class_1) / 2
