...
def active_learning_fixmatch(
    ...
    args, num_classes=2, threshold=0.95, lambda_u=1.0, epochs=200, retrain_epochs=70, batch_size=128,
    al_batch_size=128, margin=1.0
):
    ...
    for epoch in range(epochs):
        ...
        for _ in range(len(labeled_loader)):
            ...
            # or 
            # loss_u_raw = F.cross_entropy(logits_u_s, pseudo_labels, reduction='none')
            # if mask.sum() > 0:
            #     loss_u = (loss_u_raw * mask).sum() / (mask.sum() + 1e-8)
            # else:
            #     loss_u = torch.tensor(0.0, device=logits_u_s.device)
            # ...existing code...
    ...
    args.start_year, args.start_month = 2020, 7
    args.end_year, args.end_month = 2021, 12
            ...
            if args.al == True:
                ...
                for epoch in range(retrain_epochs):
                    ...
                    for _ in range(len(labeled_loader)):
                        ...
                        # Before loss_x calculation
                        if torch.isnan(x_l).any() or torch.isinf(x_l).any():
                            print("NaN or Inf in x_l")
                            break
                        if torch.isnan(y_l).any() or torch.isinf(y_l).any():
                            print("NaN or Inf in y_l")
                            break
                        if (y_l < 0).any() or (y_l >= num_classes).any():
                            print("Label out of range in y_l")
                            break

                        if torch.isnan(logits_x).any() or torch.isinf(logits_x).any():
                            print("NaN or Inf in logits_x")
                            break
...
if __name__ == "__main__":
    ...
    # Load data
    path = "/home/mhaque3/myDir/data/gen_androzoo_drebin/"
    # file_path = f"{path}2019-01to2019-12_selected.npz"
    ...
    append_to_strategy(f"_E{args.epochs}_ALE{args.retrain_epochs}")
