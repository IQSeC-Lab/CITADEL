...
def train_fixmatch_drift_eval(
    ...
    args, num_classes=2, threshold=0.95, lambda_u=1.0, epochs=250, batch_size=64
):
    ...
    for epoch in range(epochs):
        ...
        for _ in range(len(labeled_loader)):
            ...
            with torch.no_grad():
                if logits_u_w.shape[1] == 1:
                    # Binary classification with sigmoid output
                    pseudo_probs = logits_u_w.squeeze(1)
                    pseudo_labels = (pseudo_probs > threshold).long()
                    # For sigmoid, mask high-confidence samples: >0.95 (malware) or <0.05 (benign)
                    mask = ((pseudo_probs > threshold) | (pseudo_probs < (1 - threshold))).float()
                    pseudo_labels = ((pseudo_probs > threshold).long())
                    mask = (pseudo_probs > threshold).float()
                else:
    ...
    metrics_df.to_csv(f"results/baseline/{strategy}.csv", index=False)
    ...
    plot_f1_fnr(metrics_df['year'], metrics_df['f1'], metrics_df['fnr'], save_path=f"results/baseline/{strategy}_f1_fnr_plot.png")
