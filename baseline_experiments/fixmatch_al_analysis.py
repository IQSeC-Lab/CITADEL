...
# def uncertainty_sampling(model, X_label, X_test, p=2):
#     """
#     Select samples from the unlabeled set based on model uncertainty.
#     Args:
#         model: Trained model
#         X_unlabeled: Unlabeled data tensor
#         threshold: Probability threshold for selecting pseudo-labels
#     Returns:
#         Indices of selected samples and their pseudo-labels
#     """
#     # feature representation of labeled and unlabeled data
#     X_labeled_enc = model.encode(X_label)
#     X_test_enc = model.encode(X_test)

#     # calculate Lp norm distance between labeled and unlabeled data
#     distances = torch.cdist(X_test_enc, X_labeled_enc, p=p)  # L2 distance
#     # find the closest labeled sample for each unlabeled sample
#     closest_indices = torch.argmin(distances, dim=1)
#     # get the most uncertain samples based on the distance budget = 200

#     # Select 200 most uncertain samples based on the largest distances
#     # For each sample in X_test, find its minimum distance to any labeled sample
#     min_distances = torch.min(distances, dim=1).values
#     # NOTE: we can apply CADE MAD for adaptive distance thresholding
#     # Get indices of top 200 samples with largest min_distances
#     num_select = min(200, min_distances.shape[0])
#     selected_indices = torch.topk(min_distances, num_select).indices.cpu().numpy()
#     # return samples from the selected indices
#     selected_samples = X_test[selected_indices]
#     return selected_indices, selected_samples
...
def get_low_confidence_indices(model, X_test, threshold=0.7, batch_size=512):
    ...
    return low_conf_indices, confidences


# ---------- Lp-Norm Uncertainty Sampling ----------
def get_uncertain_samples(model, X_label, X_test, p=2, top_k=200):
    """
    Computes Lp-distance-based uncertainty using encoded representations.
    """
...
def active_learning_step(args, model, X_labeled, y_labeled, X_test, y_test, top_k=200, confidence_threshold=0.7):
    ...
    low_conf_indices, confidences = get_low_confidence_indices(model, X_test, threshold=confidence_threshold)
    ...
    final_selected_indices = low_conf_indices[uncertain_indices_in_lowconf]
...
# def uncertainty_sampling_lp_pseudolabel(model, X_label, y_label, X_test, p=2, num_select=200):
#     """
#     Select most uncertain samples from X_test based on Lp norm distance from X_label,
#     and assign pseudo-labels using nearest neighbor in encoded space.

#     Args:
#         model: model with an .encode() method
#         X_label: labeled data tensor
#         y_label: ground truth labels for X_label
#         X_test: unlabeled/test data tensor
#         p: Lp-norm to use
#         num_select: number of uncertain samples to select

#     Returns:
#         selected_indices: indices of selected test samples
#         selected_samples: data samples
#         pseudo_labels: pseudo-labels inferred via nearest labeled neighbors
#     """
#     # Encode feature representations
#     X_labeled_enc = model.encode(X_label)
#     X_test_enc = model.encode(X_test)

#     # Compute Lp distances between test and labeled samples
#     distances = torch.cdist(X_test_enc, X_labeled_enc, p=p)

#     # Find minimum distance for each test sample
#     min_distances, nearest_indices = torch.min(distances, dim=1)

#     # Select top-k most uncertain samples (farthest from labeled samples)
#     num_select = min(num_select, min_distances.shape[0])
#     selected_indices = torch.topk(min_distances, num_select).indices

#     # Get selected samples and their nearest neighbor labels
#     selected_samples = X_test[selected_indices]
#     neighbor_indices = nearest_indices[selected_indices]
#     pseudo_labels = y_label[neighbor_indices]

#     return selected_indices.cpu().numpy(), selected_samples, pseudo_labels
...
def active_learning_fixmatch(
    ...
    metrics_df.to_csv(f"results/al_uc/{strategy}_active.csv", index=False)
    ...
    plot_f1_fnr(metrics_df['year'], metrics_df['f1'], metrics_df['fnr'], save_path=f"results/al_uc/{strategy}_active_f1_fnr_plot.png")
    return metrics_df
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lp', default=2, type=int, help='Lp norm for uncertainty sampling (default: 2)')
