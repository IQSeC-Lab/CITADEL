...
SEED = 3  # You can set this to any integer you like
...
strategy = "freematch_bit_flip_not_mal_feat_w2_s5_r3_ep200_active"
...
def train_freematch_drift_eval(model, X_labeled, y_labeled, X_unlabeled, \
    ...
    freeMatch = FreeMatch(model_fn=model, num_classes=2, ema_momentum=0.99, \
                          lambda_unsup=1.0, lambda_fairness=0.05, ema_decay=0.99)   
    ...
    for epoch in tqdm(range(epochs), desc="Training FreeMatch"):
        ...
        for _ in tqdm(range(len(labeled_loader)), desc="FreeMatch Applying..."):
            ...
            x_u_w = utils.random_bit_flip(x_u, n_bits=2, fixed_set="notmal")
            x_u_s = utils.random_bit_flip(x_u, n_bits=5, fixed_set="notmal")
    ...
    for test_idx, year in tqdm(enumerate(sorted_test_keys), total=len(sorted_test_keys), desc="Active Learning Loop"):
        X_test, y_test, y_actual = test_sets_by_year[year]

        # === Evaluate on this test set BEFORE adding to unlabeled set ===
        metrics, mismatch_details = eval.evaluate_model_active(freeMatch.get_model(), X_test, y_test, y_actual, num_classes=num_classes)
        metrics['year'] = year
        ...
        optimizer = torch.optim.Adam(freeMatch.get_model().parameters(), lr=0.001)
        scheduler = CosineAnnealingLR(optimizer, T_max=2**20)
        ...
        for epoch in tqdm(range(retrain_epochs), desc="Retraining FreeMatch..."):
            total_loss = 0
            labeled_iter = iter(labeled_loader)
            ...
            logging.info(f"##Re-train epoch {epoch+1}/{retrain_epochs}: loss={avg_loss:.4f}, best_loss={best_loss:.4f}, sup_loss={avg_sup_loss:.4f}, unsup_loss={avg_unsup_loss:.4f}")
            # scheduler.step()
    ...
    metrics_df.to_csv(f"/home/ihossain/ISMAIL/SSL-malware/baseline_experiments/freematch/results/{strategy}_active.csv", index=False)
    ...
    plot_gen.plot_f1_fnr(metrics_df['year'], metrics_df['f1'], metrics_df['fnr'], \
                         save_path=f"/home/ihossain/ISMAIL/SSL-malware/baseline_experiments/freematch/results/f1_fnr_{strategy}_active.png")
...
# nohup python active_learning.py > /home/ihossain/ISMAIL/SSL-malware/baseline_experiments/freematch/output.log 2>&1 &