...
def active_learning_fixmatch(
    ...
    args.boundary = True
    if args.boundary == True:
        X_all = torch.cat([X_labeled, X_unlabeled], dim=0)
        y_all = torch.cat([y_labeled, y_unlabeled], dim=0)
        selected_indices, _ = select_boundary_samples(model, X_all, y_all, top_k=5000)
    ...
    metrics_list = []
    model.eval()
    path = "/home/shared-datasets/Feature_extraction/Our_experiments/LAMDA_dataset/Baseline_npz_monthwise/test/"
    args.start_year, args.start_month = 2014, 7
    args.end_year, args.end_month = 2025, 1
    ...
    for year in range(args.start_year, args.end_year + 1):
        for month in range(1, 13):
            ...
            try:
                with torch.no_grad():
                    print(f"Loading test data: {year}-{month:02d}")
                ...
                print(f"Test data for {year}-{month:02d} is not available.")
                continue
            ...
            if args.al == True:
                ...
                if args.self_training == True:
                    print("Self-training does not select any samples from Human Analyst.")
                ...
                labeled_ds = TensorDataset(X_labeled, y_labeled)
                # labeled_ds = TensorDataset(X_boundary, y_boundary)
...
if __name__ == "__main__":
    ...
    # parse arugments for uncertainty sampling option 1. lp-norm, 2. boundary selection 3. priority
    ...
    parser.add_argument('--unc_samp', type=str, default='lp-norm', choices=['lp-norm', 'boundary', 'priority'], help='Uncertainty sampling method to use')
    ...
    # Load data
    X_train = load_npz("/home/shared-datasets/Feature_extraction/Our_experiments/LAMDA_dataset/Baseline_npz/2013_X_train.npz")
    X = X_train.toarray()
    y = np.load("/home/shared-datasets/Feature_extraction/Our_experiments/LAMDA_dataset/Baseline_npz/2013_meta_train.npz", allow_pickle=True)['y']
    ...
    X_labeled, y_labeled, X_unlabeled, y_unlabeled = split_labeled_unlabeled(X, y, labeled_ratio=args.labeled_ratio)
