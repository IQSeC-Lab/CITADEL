...
def active_learning_step(prof, args, year_month, model, X_labeled, y_labeled, X_test, y_test, top_k=200):
    """
...
def active_learning_fixmatch(
    prof, bit_flip, model, optimizer, X_labeled, y_labeled, X_unlabeled, y_unlabeled,
    args, num_classes=2, threshold=0.95, lambda_u=1.0, epochs=1, retrain_epochs=1, batch_size=10,
    al_batch_size=10, margin=1.0
):
    ...
    args.end_year, args.end_month = 2013, 7
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="/home/mhaque3/myDir/data/gen_apigraph_drebin/", help="Path to the dataset")
    ...
    with torch.profiler.profile(
        schedule=torch.profiler.schedule(wait=0, warmup=0, active=100000),  # large enough
    ...
    for evt in prof.key_averages():
        total_cpu_time_us += evt.cpu_time_total
        total_cuda_time_us += evt.device_time
        total_ops += evt.count
