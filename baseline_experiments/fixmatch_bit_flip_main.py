...
# def random_bit_flip_bernoulli(x, p=0.01):
#     """
#     Randomly flip each bit in the input tensor with probability p using Bernoulli distribution.
#     Args:
#         x: Tensor of shape (batch_size, num_features)
...
#     """
#     x_aug = x.clone()
#     # Sample mask: 1 means flip, 0 means keep
#     flip_mask = torch.bernoulli(torch.full_like(x_aug, p))
#     x_aug = torch.abs(x_aug - flip_mask)  # Flip bits where mask==1
#     return x_aug


# def random_feature_mask(x, n_mask=1):
#     """
#     Randomly mask (set to 0) n_mask features in each sample of the batch.
#     Args:
#         x: Tensor of shape (batch_size, num_features)
#         n_mask: Number of features to mask per sample
#     Returns:
...
# # Combine bit flip and feature masking into a single augmentation function
# def random_bit_flip_and_mask(x, n_bits=1, n_mask=1):
#     x_aug = random_bit_flip(x, n_bits=n_bits)
#     x_aug = random_feature_mask(x_aug, n_mask=n_mask)
#     return x_aug
...
#     args, num_classes=2, threshold=0.85, lambda_u=1.0, epochs=200, batch_size=64
# ):
#     labeled_ds = TensorDataset(X_labeled, y_labeled)
#     unlabeled_ds = TensorDataset(X_unlabeled)
...
#             # x_u_w = aug_fn(x_u, n_bits=1)
#             # x_u_s = aug_fn(x_u, n_bits=bit_flip)
...
#     # Save results to CSV
#     metrics_df = pd.DataFrame(metrics_list)
#     metrics_df.to_csv(f"results/mult_aug/{strategy}.csv", index=False)

#     print(f"Mean F1 Scores: {metrics_df['f1'].mean():.4f}")
#     print(f"Mean False Negative Rates: {metrics_df['fnr'].mean()}")
#     print(f"Mean False Positive Rates: {metrics_df['fpr'].mean()}")
#     plot_f1_fnr(metrics_df['year'], metrics_df['f1'], metrics_df['fnr'], save_path=f"results/mult_aug/{strategy}_f1_fnr_plot.png")
