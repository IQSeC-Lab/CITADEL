    ...
    for epoch in range(1, total_epochs + 1):
        ...
        if args.ssl == True and (X_unlabeled is not None or pseudo_loader is not None): # ✅✅✅
            ...
            indicies, y_pseudo = get_high_similar_samples(X_train_tensor, y_train_binary_cat_tensor[:, 1], y_train_tensor, \
                                                          X_unlabeled_tensor, sm_fn=args.sm_fn, K=args.k_majority, num_samples=args.num_samples)
    ...
    for idx, (x_batch, y_batch, y_bin_batch, weight_batch) in enumerate(train_loader):
        ...
        if args.loss_func == 'triplet-mse':
            ...
            loss, supcon_loss, xent_loss = HiDistanceXent(args.xent_lambda, \
                                            ...
                                            margin = args.margin, \
                                            weight = weight_batch)
            # BCELoss = BinaryCrossEntropy().to(device=device)
            # # logging.info(f'y_pred: {y_pred}, y_bin_batch: {y_bin_batch}')
            # loss = BCELoss(y_pred, y_bin_batch)
...
def train_encoder_one_epoch1(args, encoder, train_loader, optimizer, epoch):
    """ Train one epoch for the model """
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
